...
3. Run validations through `.\dev.ps1` (contract/status in `spec/development-workflow.md §4`). Implemented commands: `doctor` (read-only); scoped harness commands `build|test|verify rational-time` (M0-002), `build|test|verify entity-revision` (M0-003), and `build|test|verify schema-foundation` (M0-004) (local CMake/CTest only; require cmake >= 3.20 and ctest on PATH; `dev.ps1` initializes the existing local MSVC Build Tools C++ environment when its build cache selects MSVC but the process lacks the MSVC environment, and never downloads/installs a toolchain). For `build|test|verify`, exactly one scope argument is accepted: a missing scope yields a **Usage** error, an unknown scope yields an **Unknown scope** error, and extra arguments are rejected — all exit non-zero. Each `verify <scope>` additionally runs the control-plane dispatch checks (`tests/dev/dispatch.tests.ps1`, negative-path mode). Every other verb/scope reports `NotImplemented:` or a prerequisite message and exits non-zero — never report them as passed; record them as skipped/not-implemented with the reason. Do not substitute ad-hoc shell commands for project checks.
